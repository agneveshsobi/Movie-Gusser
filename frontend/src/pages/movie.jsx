...
function Movie() {
  ...
  const game = MOVIES[currentIndex]
  const maxWrong = 4
  const blurAmount = Math.max(0, 20 - cluesRevealed * 2)
...
const handleGuess = async () => {
    ...
    if (normalize(guess) === normalize(game.title)) {
      ...
      if (token) {
        try {
          await saveScore({ category: "movie", points: points })  // ← "movie" not "anime"
        } catch (err) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ color: "#6366f1" }}> Score: {score}</span>
      </div>
